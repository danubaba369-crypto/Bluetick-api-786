...
const WabaRequiredModal: React.FC<WabaRequiredModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md! max-w-[calc(100%-2rem)]! max-h-[90vh]! custom-scrollbar overflow-y-auto! border-none shadow-2xl p-0! bg-white dark:bg-(--card-color) rounded-xl">
        ...
        <div className="sm:p-6 p-4 flex flex-col items-center text-center space-y-6">
          ...
          <div className="w-full pt-4 space-y-3">
            <Button
              className="w-full h-12 bg-primary text-white font-medium text-[15px] rounded-lg shadow-lg shadow-primary/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group"
