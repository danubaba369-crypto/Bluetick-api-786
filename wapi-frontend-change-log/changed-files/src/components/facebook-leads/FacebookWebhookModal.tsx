...
const FacebookWebhookModal = ({ isOpen, onClose }: FacebookWebhookModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md! max-w-[calc(100%-2rem)]! max-h-[90vh] custom-scrollbar gap-0! p-0! overflow-auto border-none shadow-2xl bg-white dark:bg-(--card-color)">
        ...
        <div className="sm:p-6 p-4 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="bg-gray-50 dark:bg-(--page-body-bg) border-gray-200 dark:border-(--card-border-color) text-gray-600 dark:text-gray-300 h-12 focus-visible:outline-none focus-visible:ring-0 font-mono text-[13px]"
                />
                <Button
                  variant="outline"
                  size="icon"
                  disabled={!webhookUrl}
                  className="shrink-0 h-12 w-12 border-gray-200 dark:border-(--card-border-color) text-gray-400 hover:text-primary hover:border-primary/30 transition-all"
          ...
          <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/20 flex gap-3">
            ...
            <p className="text-[11px] text-amber-700 dark:text-amber-400/80 leading-relaxed">
              Use these credentials to configure the Webhook section in your Meta App Settings. This allows Wapi to receive real-time updates when users submit your Facebook Lead Forms.
            </p>
          </div>
        </div>

        <div className="p-4 bg-gray-50 dark:bg-(--card-color) border-t border-gray-100 dark:border-(--card-border-color) flex justify-end">
          <Button onClick={onClose} className="bg-primary hover:bg-primary/90 text-white min-w-25">
            {t("common_close")}
          </Button>
        </div>
      </DialogContent>
