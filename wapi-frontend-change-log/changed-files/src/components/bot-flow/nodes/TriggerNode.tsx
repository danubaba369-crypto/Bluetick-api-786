      ...
      <NodeField label="Activation Method" required error={(touched || data.forceValidation) && !data.triggerType ? "Please select a rule" : ""}>
        <div className="grid grid-cols-2 gap-2 mt-1">
          {[
            ...
            <Button key={rule.value} type="button" variant="outline" onClick={() => updateNodeData("triggerType", rule.value)} className={cn("flex flex-col items-center justify-center h-[82px] p-2 rounded-lg transition-all duration-300 border h-16 relative group overflow-hidden", data.triggerType === rule.value ? "border-indigo-500 bg-indigo-500/5 ring-1 ring-indigo-500/20 shadow-sm" : "border-gray-100 bg-gray-50/50 hover:border-gray-200 dark:border-(--card-border-color) dark:bg-white/5 dark:hover:bg-(--table-hover) dark:hover:border-(--card-border-color)")}>
