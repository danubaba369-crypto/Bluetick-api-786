...
const LivePreview: React.FC<LivePreviewProps> = ({ allFields, submitSettings }) => {
  ...
  return (
    <div className="w-full max-w-77.5 mx-auto h-130 sm:h-145 bg-[url('/assets/images/1.png')] bg-cover bg-center bg-no-repeat [scrollbar-width:none] [&::-webkit-scrollbar]:size-0 border-4 border-black/90 dark:border-slate-800 rounded-[2.2rem] overflow-hidden shadow-2xl relative flex flex-col">
        ...
        {isPreviewOpen && (
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="absolute inset-0 top-22 z-50 flex flex-col bg-slate-50 dark:bg-(--page-body-bg) rounded-t-3xl overflow-hidden">
            ...
            <div className="bg-white dark:bg-(--page-body-bg) border-b border-slate-100 dark:border-(--card-border-color) px-4 py-2 flex items-center justify-between shrink-0 shadow-sm">
              <Button onClick={handleReset} className="w-8 h-8 rounded-full hover:bg-slate-100 dark:hover:bg-(--table-hover) flex items-center justify-center text-slate-500 transition-colors">
                <X size={18} />
              </Button>
              <h3 className="text-sm font-bold text-slate-900 dark:text-gray-500">Form Preview</h3>
              <Button className="w-8 h-8 rounded-full hover:bg-slate-100 dark:hover:bg-(--table-hover) flex items-center justify-center text-slate-500 transition-colors">
                <MoreVertical size={18} />
              </Button>
            </div>
