...
const Footer: React.FC<FooterProps> = ({ data }) => {
  ...
  return (
    <footer className="footer rounded-t-lg px-6 md:px-12 lg:px-[calc(20px+223*((100vw-320px)/1600))] pt-12 pb-8 text-white bg-landing-theme-dark">
      <div className="flex flex-col md:flex-row justify-between items-center gap-[calc(16px+16*((100vw-320px)/1600))] pb-8 border-b border-white/10">
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-3">
            <Images src={logo_dark_url || Logo1} alt={`${app_name || t("app_name")} logo`} className="h-7.75 object-contain" width={100} height={100} />
          </Link>
        </div>
        ...
        <div className="flex gap-6.25 flex-wrap justify-end">
          <div className="flex flex-wrap justify-center md:justify-end gap-x-8 gap-y-4">
            {socialLinksArray.map(({ icon, label, href }) => (
              <a key={label} href={href} target="_blank" rel="noopener noreferrer" className="group transition duration-300 flex items-center gap-2 text-sm text-white hover:text-primary">
      ...
      <div className="relative flex flex-col lg:flex-row gap-y-[calc(16px+48*((100vw-320px)/1600))] lg:gap-y-0">
        ...
        <div className="flex flex-col gap-[calc(8px+20*((100vw-320px)/1600))] pt-10 lg:pr-16 w-full lg:w-5/12 shrink-0 [@media(max-width:576px)]:justify-center [@media(max-width:576px)]:text-center [@media(max-width:576px)]:items-center">
          <h2 className="text-[calc(20px+26*((100vw-320px)/1600))] font-extrabold leading-tight whitespace-pre-wrap">{data.cta_title}</h2>
          ...
          <div className="flex gap-3 flex-wrap [@media(max-width:347px)]:justify-center">
            {data.cta_buttons?.map((btn, idx) => (
              <Button
                key={idx}
                className={idx === 0 ? "h-11! bg-primary! hover:bg-primary! text-white! font-bold! text-sm! py-3! px-7! border-none! rounded-xl! transition-colors" : "bg-transparent! border-2! border-primary! h-11! hover:bg-primary! text-white! font-bold! text-sm! py-3! px-7! rounded-xl! transition-colors"}
                onClick={() => {
                  if (isAuthenticated) {
      ...
      <div className="mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
        <p className="text-[18px] text-white">© {data.copy_rights_text || `${app_name || t("app_name")} 2026. All Rights Reserved.`}</p>
