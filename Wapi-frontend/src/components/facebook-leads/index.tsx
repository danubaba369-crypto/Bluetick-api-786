...
import { CheckCircle2, Globe, Layout, Link, Trash2, XCircle } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import { useEffect, useMemo, useState } from "react";
import { useTranslation } from "react-i18next";
import { toast } from "sonner";
import ConnectFormModal from "./ConnectFormModal";
import FacebookWebhookModal from "./FacebookWebhookModal";
...
const FacebookLeads = () => {
  ...
  const rightContent = (
    <div className="flex items-center gap-4 flex-wrap justify-end">
      ...
      <Button variant="outline" className="h-12 px-4 gap-2 bg-white dark:bg-(--card-color) border-slate-200 dark:border-(--card-border-color) text-slate-600 dark:text-gray-300 rounded-lg font-semibold transition-all shadow-sm hover:bg-slate-50 dark:hover:bg-(--table-hover)" onClick={() => setIsWebhookModalOpen(true)}>
        <Globe size={18} className="text-primary" />
        <span className="hidden sm:inline">{t("facebook_webhook")}</span>
      </Button>
  ...
  return (
    <div className="sm:p-8 pt-0! p-4 space-y-8 bg-(--page-body-bg) dark:bg-(--dark-body)">
      ...
      <FacebookWebhookModal isOpen={isWebhookModalOpen} onClose={() => setIsWebhookModalOpen(false)} />
    </div>
  );
};
