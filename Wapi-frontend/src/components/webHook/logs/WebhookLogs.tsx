...
const WebhookLogs = () => {
  ...
  const getStatusBadge = (status: string) => (
    <Badge variant="outline" className={status === "success" ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20" : "bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-100 dark:border-red-500/20"}>
      <div className="flex items-center gap-1.5 px-1 font-bold">
        {status === "success" ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
  ...
  const triggerColumns: Column<WebhookLog>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-red-500 text-xs font-medium max-w-50 truncate block" title={row.error_message}>
          {row.error_message || "-"}
        </span>
      ),
    },
    {
      ...
      cell: (row) => <span className="text-slate-500 dark:text-slate-400 text-xs font-medium">{formatDateTime(row.created_at)}</span>,
    },
    {
      header: "Payload",
  ...
  const messageColumns: Column<WebhookLog>[] = [
    ...
    },
    {
      ...
      accessorKey: "error_message",
      cell: (row) => (
        <span className="text-red-500 text-xs font-medium  block break-word whitespace-normal" title={row.error_message}>
          {row.error_message || "-"}
        </span>
      ),
    },
