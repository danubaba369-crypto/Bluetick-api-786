...
const Sidebar = ({ onMenuClick }: SidebarProps) => {
  ...
  const { isFeatureEnabled, isLoading: featureLoading } = useFeatureAccess();
  ...
  const filteredMenuItems = MENUITEMS.filter((item) => {
    ...
    // 2. Feature Toggle check (Plan based)
    if (item.featureKey && !isFeatureEnabled(item.featureKey)) {
      return false;
    }

    // 3. Hide Tasks from sidebar for non-agents
    if (item.label === "tasks" && user?.role !== "agent") {
      return false;
    }

    // 4. Baileys restriction
  ...
  const sections = ["overview_sidebar_title", "setup_integration_sidebar_title", "communication_sidebar_title", "contacts_audience_sidebar_title", "automation_sidebar_title", "operations_sidebar_title", "marketing_sidebar_title", "tools_sidebar_title", "team_management_sidebar_title", "billing_settings_sidebar_title"];
  ...
  const sidebarLogo = useMemo<string | null>(() => {
    ...
    if (isVisuallyExpanded) {
      const url = isDark ? logo_light_url : logo_dark_url;
      return resolveUrl(url) || (isDark ? "/assets/logos/logo1.png" : "/assets/logos/logo3.png");
    }

    const url = isDark ? sidebar_light_logo_url : sidebar_dark_logo_url;
  ...
  const [openSections, setOpenSections] = useState<string[]>(["overview_sidebar_title", "setup_integration_sidebar_title", "communication_sidebar_title", "contacts_audience_sidebar_title", "automation_sidebar_title", "operations_sidebar_title", "marketing_sidebar_title", "tools_sidebar_title", "team_management_sidebar_title", "billing_settings_sidebar_title"]);
    ...
    <div className="h-full flex flex-col bg-white dark:bg-(--card-color) border-r border-gray-200 dark:border-(--card-border-color)">
      <div className={`flex items-center justify-center h-18 border-b border-gray-200 dark:border-(--card-border-color) transition-all duration-300 ${isVisuallyExpanded ? "px-6" : "px-3"}`}>
        <div className={`flex items-center w-full overflow-hidden justify-between`}>
          <div className="flex items-center gap-3 overflow-hidden">{sidebarLogo === null ? <div className="h-10 w-28 bg-gray-200 dark:bg-white/10 animate-pulse rounded-md" /> : <Image src={sidebarLogo} alt={app_name || "logo"} width={160} height={48} unoptimized className={`max-h-12 object-contain transition-all duration-300 ${isVisuallyExpanded ? "w-auto" : "w-full max-w-10"}`} />}</div>
