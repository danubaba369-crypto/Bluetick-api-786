    ...
    <div className="flex h-[calc(100vh-75px)] bg-white dark:bg-(--card-color) overflow-hidden relative">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-(--card-color) flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {selectedGuide ? (
            <div className="max-w-6xl mx-auto px-8 md:px-12 py-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                      ...
                      <div className={`grid gap-8 mt-10 ${(section.images?.length || 0) > 1 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}>
                        {section.images.map((img, imgIdx) => (
                          <figure key={imgIdx} className="space-y-4 group">
                            <div className="relative rounded-3xl overflow-hidden border border-slate-100 dark:border-white/5 shadow-2xl shadow-slate-200/50 dark:shadow-none bg-slate-50 dark:bg-white/2">
                            ...
                            {img.caption && <figcaption className="text-xs text-center text-slate-400 font-bold uppercase tracking-widest">{img.caption}</figcaption>}
                          </figure>
                        ))}
                      </div>
                    )}
