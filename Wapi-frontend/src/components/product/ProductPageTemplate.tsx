...
const HeroSection: React.FC<ProductPageTemplateProps["hero"]> = ({ title, description, primaryCTA, secondaryCTA, image }) => {
  ...
  return (
    <section className="relative py-[calc(25px+47*((100vw-320px)/1600))] overflow-hidden bg-white">
      <div className="container mx-auto px-[calc(14px+10*((100vw-320px)/1600))]">
        <div className="flex flex-col lg:flex-row items-center gap-[calc(20px+44*((100vw-320px)/1600))]">
          <motion.div initial={{ opacity: 0, x: -25 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="flex-1 text-center lg:text-left z-10">
            ...
            <h1 className="text-[calc(18px+23*((100vw-320px)/1600))] font-black text-slate-900 leading-[1.05] mb-[calc(14px+18*((100vw-320px)/1600))] tracking-tight">{title}</h1>
            <p className="text-[calc(14px+4*((100vw-320px)/1600))] text-slate-600 mb-[calc(16px+26*((100vw-320px)/1600))] max-w-2xl mx-auto lg:mx-0 leading-relaxed font-medium">{description}</p>
            <div className="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-4 mb-[calc(16px+24*((100vw-320px)/1600))]">
              {primaryCTA && (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.1 }} className="flex-1 w-full lg:w-auto relative ">
            <Images src={image} alt="Product Interface" width={800} height={500} unoptimized className="w-full h-auto rounded-lg" />
          </motion.div>
        </div>
      </div>
    </section>
...
const FeaturesSection: React.FC<{ features: FeatureBlock[] }> = ({ features }) => (
  <section className="py-[calc(25px+47*((100vw-320px)/1600))] bg-white">
    <div className="container mx-auto px-[calc(14px+10*((100vw-320px)/1600))] flex flex-col gap-[calc(36px+28*((100vw-320px)/1600))]">
      {features.map((feature, idx) => (
        <div key={idx} className={`flex flex-col lg:flex-row items-center gap-[calc(20px+44*((100vw-320px)/1600))] ${idx % 2 === 1 ? "lg:flex-row-reverse" : ""}`}>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} className="flex-1">
            ...
            <p className="text-[calc(14px+6*((100vw-320px)/1600))] text-slate-600 mb-[calc(22px+26*((100vw-320px)/1600))] leading-relaxed font-normal">{feature.description}</p>
