...
export const OTPVerificationPage = ({ type = "forgot_password" }: OTPVerificationPageProps) => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-emerald-50 flex items-center justify-center p-2 sm:p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl">
        <div className="grid lg:grid-cols-2 gap-0 items-center">
              ...
              <div className="space-y-4">
                {isAuthSetupLoading ? (
                  Array(2).fill(0).map((_, i) => (
                    <div key={i} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-5">
                  ...
                  content.side_panel.bullets.map((bullet, index) => {
                    ...
                    return (
                      <div key={index} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-5">
                        <div className="flex items-start gap-4">
          ...
          <div className="bg-white lg:rounded-r-3xl shadow-2xl px-4 py-8 sm:p-12 h-full flex flex-col justify-center relative">
            ...
            <div className="relative z-10 max-w-md mx-auto w-full">
              ...
              <div className="mb-8">
                <h2 className="text-2xl sm:text-3xl text-slate-900 font-bold mb-3">
                  {isAuthSetupLoading ? <Skeleton className="h-9 w-48" /> : content.title}
                </h2>
                <div className="text-slate-600">
                  {isAuthSetupLoading ? <Skeleton className="h-5 w-64" /> : <>{content.subtitle} <span className="font-semibold text-slate-900">{identifier}</span></>}
                </div>
              </div>
              ...
              <form onSubmit={formik.handleSubmit}>
                  ...
                  <div className="flex sm:gap-3 gap-1.5 justify-center" onPaste={handlePaste}>
                    {otp.map((digit, index) => (
                      <Input
                        ...
                        className="sm:w-14 sm:h-14 w-9 h-9 text-center sm:text-2xl text-xl font-bold border border-(--input-border-color) rounded-lg focus:border-primary focus:ring-4 focus:ring-emerald-100 outline-none transition-all bg-slate-50 focus:bg-white"
                      />
                    ))}
                  </div>
                ...
                <Button type="submit" className="w-full h-13 bg-primary text-white rounded-lg shadow-lg shadow-emerald-500/30 transition-all text-base font-semibold" disabled={isLoading || otp.some((d) => !d) || isAuthSetupLoading}>
                  {isLoading ? (
              ...
              {type === "signup" && !isAuthSetupLoading && (
                <div className="mt-8 p-4 bg-emerald-50/50 border border-emerald-100 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center shrink-0 shadow-sm">
                    {isWhatsApp ? <MessageCircle className="w-5 h-5 text-emerald-600" /> : <Mail className="w-5 h-5 text-emerald-600" />}
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-emerald-700 leading-relaxed">
                      {t("check_your_delivery_method", { method: isWhatsApp ? "WhatsApp" : t("email_address") })}: <span className="font-bold text-emerald-800 underline decoration-emerald-200 decoration-2 underline-offset-2">{identifier}</span>
                    </p>
                  </div>
                </div>
              )}
